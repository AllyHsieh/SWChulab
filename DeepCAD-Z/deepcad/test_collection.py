        ...
        for pth_index in range(len(self.model_list)):
            ...
            if '.pth' in aaa:
                ...
                for N in range(len(self.img_list)):
                    ...
                    for iteration, (noise_patch, single_coordinate) in enumerate(testloader):
                        ...
                        if (postprocess_turn > 1):
                            for id in range(postprocess_turn):
                                ...
                                raw_image += img_mean
                                denoise_img  = output_image * (np.sum(raw_image) / np.sum(output_image)) ** 0.5
                                input_img = raw_image+img_mean
                        else:
                            output_image += img_mean
                            raw_image += img_mean
                            denoise_img  = output_image * (np.sum(raw_image) / np.sum(output_image)) ** 0.5
                    ...
                    # Normalize and display inference imagesinglebatch_test_save
                    ...
                    if (self.save_test_images_per_epoch):
                        ...
                        result_name = output_path_name + '//' + self.img_list[N].replace('.tiff','') + '_denoised.tiff'
