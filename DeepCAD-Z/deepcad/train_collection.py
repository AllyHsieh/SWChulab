...
class training_class():
    ...
    def train_preprocess_lessMemoryMulStacks(self):
        ...
        for im_name in list(os.walk(self.datasets_path, topdown=False))[-1][-1]:
            ...
            # self.get_gap_t()
            self.gap_x = self.patch_x
            self.gap_y = self.patch_y
            ...
            for x in range(0, int((self.whole_y) / self.gap_y)):
                for y in range(0, int((self.whole_x) / self.gap_x)):
                    for z in range(0, int((self.whole_t) / self.gap_t)):
    ...
    def train(self):
        ...
        for epoch in range(0, self.n_epochs):
            ...
            trainloader = DataLoader(train_data, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers)
            ...
            for iteration, (input, target) in enumerate(trainloader):
                ...
                if iteration % 1 == 0:
                    ...
                    mae_list = np.append(mae_list, L1_loss.item())
                    mse_list = np.append(mse_list, L2_loss.item())
                    print(
            ...
            loss_rec = np.append(loss_rec, np.mean(loss_list))
            mae_rec = np.append(mae_rec, np.mean(mae_list))
            mse_rec = np.append(mse_rec, np.mean(mse_list))
    ...
    def test(self, train_epoch, train_iteration):
        ...
        testloader = DataLoader(test_data, batch_size=self.batch_size, shuffle=True, num_workers=self.num_workers)
