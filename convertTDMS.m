filename='E:\20240325\20xNA0.5_mouse_HWP306_PMT0.8_dt0.005_z0-3-300_StackImage.tdms';
[ConvertedData,ConvertVer,ChanNames,GroupNames,ci]=convertTDMS(0,filename); % 1:save, 0:no save

for i=3:3+findDI0(ConvertedData.Data.MeasuredData)
csv=ConvertedData.Data.MeasuredData(i).Data;
a=reshape(csv,[512,512]);
img(:,:,i-2)=flipud(a');
end
savefile(0,img);  %1:save, 0:no save
 
function savefile(flag,data)
if flag==1
[pathstr, name, ext] = fileparts(filename);
matfilename = fullfile(pathstr, [name '.mat']);
save(matfilename, 'data');
else
disp('Data saving is skipped.');
end
end
function last=findDI0(dataname)
numbers=[];
for i = 1:length(dataname)
    currentName = dataname(i).Name;
    token = regexp(currentName, 'StackImageDI0/(\d+)', 'tokens');
    if ~isempty(token)
        if str2double(token{1})~=0
        numbers(end+1) = str2double(token{1});
        end
    end
end
last=length(numbers);
